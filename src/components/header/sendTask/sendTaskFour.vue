<template>
  <div class="sendTaskFour">
    ...
    <div class="cont">
      <h1>支付成功，任务已发布等待客服审核</h1>
      <h3>{{ time }}s后自动返回任务详情
        <el-button type="text" @click="lookTask">&nbsp;&nbsp;立即查看 ></el-button>
      </h3>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    lookTask () {
      this.$router.push({ name: 'taskDetail', query: { sellerTaskId: this.$route.query.sellerTaskId || this.$route.query.rbSellerTaskId } })
    }
  },
  mounted () {
    this.timeOn()
